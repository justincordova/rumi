  ...
  return (
    <div className="flex items-center -space-x-2">
      {visible.map((u) => (
        <Avatar
          key={u.user_id}
          title={u.isGuest ? "Guest" : (u.display_name ?? "Anonymous")}
      ...
      {overflow > 0 && (
        <div className="flex h-7 w-7 items-center justify-center rounded-full bg-muted ring-2 ring-background text-[10px] font-medium text-muted-foreground shrink-0 select-none">
          +{overflow}
        </div>
      )}
    </div>
